(function(){
    ...
    async function startRecording() {
      ...
      if(!connectionStatusCheck()) {
        addLogMessage(`⚠️ No peer connected. Share your ID: ${myShortId} & connect first!`, false, 'text');
        return;
      }
      pttLock = true;
      try {
    ...
    function sendTextMessage() {
      ...
      if(!connectionStatusCheck()) { addLogMessage(`⚠️ No peer connected. Share your ID: ${myShortId} & connect first.`, false, 'text'); return; }
      const fullMsg = `✉️ ${msg}`;
    ...
    function connectToPeer(shortId) {
      ...
      const fullRemoteId = "okt_" + shortId.toUpperCase();
    ...
    function initPeer() {
      ...
      peer.on('disconnected', () => { 
        addLogMessage(`📡 Signaling lost, reconnecting with same ID...`, false, 'text'); 
        peer.reconnect(); 
      });
    }
    ...
    function attachPTTEvents() {
      const startTalk = (e) => { e.preventDefault(); if(!connectionStatusCheck()) { addLogMessage(`⚠️ No peer connected. Share your ID: ${myShortId}`, false, 'text'); return; } startRecording(); };
